...
sampler2D _LightTextureB0;
	...
	UnityLight CreateLight(float2 uv, float3 worldPos, float viewZ)
	{
		...
		bool shadowed = false;
			...
			attenuation *= tex2D(_LightTextureB0, dot(lightVec, lightVec) * _LightPos.w).UNITY_ATTEN_CHANNEL;
				...
				attenuation *= uvCookie.w < 0;
					...
					shadowed = true;
					shadowAttenuation 
					= UnitySampleShadowmap(mul(unity_WorldToShadow[0], float4(worldPos, 1)));
				#endif
			#else
				#if defined(POINT_COOKIE)
		...
		if(shadowed)
		{
			...
			shadowAttenuation = 
				UnityMixRealtimeAndBakedShadows(shadowAttenuation, GetShadowMaskAttenuation(uv), shadowFade);
